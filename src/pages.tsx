...
import * as reactjsLogo from './components/shapes/reactjs-logo';
...
const tipItems: any = [flexMultiRow, lineClamping, textOverflow];
...
featureItems.forEach((item: any) =>
  pushItemToPages(GROUP_PAGE.FEATURE, item),
);
tipItems.forEach((item: any) =>
  pushItemToPages(GROUP_PAGE.TIP, item),
);
shapeItems.forEach((item: any) =>
  pushItemToPages(GROUP_PAGE.SHAPE, item),
);
